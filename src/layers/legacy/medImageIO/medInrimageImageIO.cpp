...
namespace itk
{
    ...
    InrimageImageIO::InrimageImageIO()
    {
        ...
        // Byte order
        this->SetByteOrder((ByteSwapper<int>::SystemIsBigEndian() == true) ? BigEndian : LittleEndian);
    ...
    void InrimageImageIO::PrintSelf(std::ostream& os, Indent indent) const
    {
    ...
    bool InrimageImageIO::CanReadFile(const char* FileNameToRead)
    {
        ...
        if (m_file == NULL)
        {
            ...
            if (!fileExt.compare(".inr.gz"))
            {
                ...
                m_file = ::gzopen(ImageFileName.c_str(), "rb");
                if (m_file == NULL)
                {
                    return false;
                }
    ...
    void InrimageImageIO::ReadImageInformation()
    {
        ...
        // file endianess
        // default = Sytem
        if (ByteSwapper<int>::SystemIsBigEndian() == true)
        ...
        while (std::getline(issh, line)) {
            ...
            case 'P':
                if (std::string(id, 0, strlen("PIXSIZE")).compare("PIXSIZE") == 0) {
                    ...
                    // cerr << pixsize << s << endl;
                    if ((pixsize == 0) || (pixsize % 8 != 0) ||
                        (std::string(s, 0, strlen("bits")).compare("bits") != 0)) {
        ...
        {
            yav::RotVec3D rotvec(rx, ry, rz);
            yav::RotMat3D rotmat;
            RotVec3DToRotMat3D(rotvec, rotmat);

            vnl_vector<double> d(3);

            d[0] = fabs(rotmat(0, 0)) > 1e-10 ? rotmat(0, 0) : 0;
            d[1] = fabs(rotmat(1, 0)) > 1e-10 ? rotmat(1, 0) : 0;
            d[2] = fabs(rotmat(2, 0)) > 1e-10 ? rotmat(2, 0) : 0;
            this->SetDirection(0, d);

            d[0] = fabs(rotmat(0, 1)) > 1e-10 ? rotmat(0, 1) : 0;
            d[1] = fabs(rotmat(1, 1)) > 1e-10 ? rotmat(1, 1) : 0;
            d[2] = fabs(rotmat(2, 1)) > 1e-10 ? rotmat(2, 1) : 0;
            this->SetDirection(1, d);

            d[0] = fabs(rotmat(0, 2)) > 1e-10 ? rotmat(0, 2) : 0;
            d[1] = fabs(rotmat(1, 2)) > 1e-10 ? rotmat(1, 2) : 0;
            d[2] = fabs(rotmat(2, 2)) > 1e-10 ? rotmat(2, 2) : 0;
        ...
        ::Write(const void* buffer)
    {
        ...
        if (fileExt == (".inr.gz"))
        {
            ...
            if (m_file == NULL)
                throw ExceptionObject(__FILE__, __LINE__, "Error in opening file for writing");
        }
        else
        {
            if (fileExt == (".inr"))
            {
                ...
                throw ExceptionObject(__FILE__, __LINE__, "Unrecognized extension.");
        }
        ...
        default:
            throw ExceptionObject(__FILE__, __LINE__, "Error: pixel type not recognized.");
        }
        ...
        else
        {
            ...
            yav::Vec3<double> v0(d[0], d[1], d[2]);

            d = this->GetDirection(1);
            yav::Vec3<double> v1(d[0], d[1], d[2]);

            d = this->GetDirection(2);
            yav::Vec3<double> v2(d[0], d[1], d[2]);
            yav::RotMat3D mat(v0, v1, v2);

            RotMat3DToRotVec3D(mat, r);
        }
        ...
        if (npix != strlen(buf))
        {
            if (isGz)
                ...
                fclose((FILE*)m_file);
            throw ExceptionObject(__FILE__, __LINE__, "Error: bad number of bytes written.");
        }

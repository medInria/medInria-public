...
void medDatabaseImporter::index(void)
{
    ...
    foreach (QString file, fileList)
    {
        ...
        dtkData = tryReadImage(fileInfo.filePath(), readOnlyImageInformation);
    ...
    for (it; it != imagesToWriteMap.end(); it++)
    {
        ...
        QStringList filePaths = it.value(); // input files being aggregated, might be only one or many

        dtkAbstractData* imageDtkData = NULL;

        QFileInfo imagefileInfo(filePaths[0]);

        // 3.2) Try to read the whole image, not just the header
        bool readOnlyImageInformation = false;
        imageDtkData = tryReadImage(imagefileInfo.filePath(), readOnlyImageInformation);
        ...
        if (imageDtkData)
        {
            ...
            addAdditionalMetaData(imageDtkData, aggregatedFileName, filePaths);
           }
        else
        {
            qWarning() << "Could not repopulate data!";
            emit showError(this, tr ("Could not read data: ") + filePaths[0], 5000);
            continue;
        }
        ...
        if (!writeSuccess){
            emit showError(this, tr ("Could not save data file: ") + filePaths[0], 5000);
            continue;
        }
...
dtkAbstractData* medDatabaseImporter::tryReadImage(QString filePath, bool readOnlyImageInformation)
{
    dtkAbstractData* dtkData = 0;

    dtkAbstractDataReader* dataReader = getSuitableReader(QStringList(filePath));

    if (dataReader)
    {
        if (readOnlyImageInformation)
        {
            dataReader->readInformation( filePath );
        }
        else
        {
    ...
    {
        qWarning() << "No suitable reader found for file: " << filePath << ". Unable to import!";
    }

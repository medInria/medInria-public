/* itkProcessRegistrationSuperBalooToolBox.cpp ---
 *
 * Author: Julien Wintz
...
public:
    ...
    QDoubleSpinBox * varianceElasticReg;
    QDoubleSpinBox * varianceFluidReg;
...
itkProcessRegistrationSuperBalooToolBox::itkProcessRegistrationSuperBalooToolBox(
        QWidget *parent) : medRegistrationAbstractToolBox(parent),
    d(new itkProcessRegistrationSuperBalooToolBoxPrivate)
{
...
//                                  "should not be used"),Qt::ToolTipRole);
    ...
    d->minimalVariance->setValue(25.0);
...
//    advancedBox->setToolTip(tr("Click to see advanced parameters."));
    ...
    d->alpha = new QDoubleSpinBox(widget);
    d->alpha->setMinimum(0.0);
    d->alpha->setSingleStep(0.01);
    d->alpha->setValue(3.0);
    d->alpha->setToolTip(
                tr("Parameter used during the outlier rejection process. "
                   "This parameter allows more variable displacements when "
                   "its value is large, and highly "
                   "constrains the pairings when it is small. (default 3.0)"));

    d->varianceElasticReg = new QDoubleSpinBox(widget);
    d->varianceElasticReg->setMinimum(0.0);
    d->varianceElasticReg->setSingleStep(0.01);
    d->varianceElasticReg->setValue(4.0);
    d->varianceElasticReg->setToolTip(
                               tr("Variance (in voxel unit) of the displacement field "
                                  "regularization (Gaussian smoothing) (default 4.0)."));

    d->varianceFluidReg = new QDoubleSpinBox(widget);
    d->varianceFluidReg->setMinimum(0.0);
    d->varianceFluidReg->setSingleStep(0.01);
    d->varianceFluidReg->setValue(4.0);
    d->varianceFluidReg->setToolTip(
                               tr("Variance (in voxel unit) of the update displacement field "
                                  "regularization (Gaussian smoothing) (default 4.0)."));


    d->processorsBox = new QSpinBox(advancedWidgets);
    d->processorsBox->setRange(1,itk::MultiThreader::GetGlobalDefaultNumberOfThreads());
    d->processorsBox->setValue(itk::MultiThreader::GetGlobalDefaultNumberOfThreads());
    ...
    advancedformLayout->addRow(tr("Alpha"),d->alpha);
    advancedformLayout->addRow(tr("Variance Fluid Regularization"),d->varianceFluidReg);
    advancedformLayout->addRow(tr("Variance Elastic Regularization"),d->varianceElasticReg);

    advancedformLayout->addRow(tr("Processors"),d->processorsBox);

    this->setTitle(tr("Super Baloo"));
...
itkProcessRegistrationSuperBalooToolBox::~itkProcessRegistrationSuperBalooToolBox(void)
{
    delete d;
...
bool itkProcessRegistrationSuperBalooToolBox::registered(void)
{
    return medToolBoxFactory::instance()->registerToolBox<itkProcessRegistrationSuperBalooToolBox>();
}

void itkProcessRegistrationSuperBalooToolBox::run(void)
{
    ...
    if (this->parentToolBox()->process() && (this->parentToolBox()->process()->identifier() == "itkProcessRegistrationSuperBaloo"))
    {
    ...
    {
        process = new itkProcessRegistrationSuperBaloo;
    ...
    itkProcessRegistrationSuperBaloo *process_reg =
            dynamic_cast<itkProcessRegistrationSuperBaloo *>(process);
    if (!process_reg)
    {
    ...
    process_reg->setStepSize(d->stepSizeX->value(),
                             d->stepSizeY->value(),
                             d->stepSizeZ->value());
    process_reg->setAlpha(d->alpha->value());
    process_reg->setElasticVarianceRegularisation(d->varianceElasticReg->value());
    process_reg->setFluidVarianceRegularisation(d->varianceFluidReg->value());
    process_reg->setProcessors(d->processorsBox->value());
    // process->setMyWonderfullParameter(fronTheGui);

    // or ...

    // itkProcessRegistrationSuperBaloo *process = new itkProcessRegistrationBaloo;
    // process->setMyWonderfullParameter(fronTheGui);
    // process->setMyWonderfullParameter(fronTheGui);
...
void itkProcessRegistrationSuperBalooToolBox::onAdvancedBoxToggled(bool toggle)
{

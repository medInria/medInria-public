...
bool Frame::Create(wxFrame * parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style )
{
	...
	item = new wxMenuItem(m_editMenu, MENU_SET_ALL_TABS_SHAPE_ANGLE, wxT("Set an inclination of tab header borders"), wxT("Set the shape of tab header"), wxITEM_NORMAL);
	m_editMenu->Append(item);
	...
	m_editMenu->Append(-1, wxT("Tabs Style"), styleMenu);
	
	item = new wxMenuItem(styleMenu, MENU_SELECT_GRADIENT_COLOR_FROM, wxT("Select fancy tab style 'from' color"), wxT("Select fancy tab style 'from' color"), wxITEM_NORMAL);
	m_editMenu->Append(item);

	item = new wxMenuItem(styleMenu, MENU_SELECT_GRADIENT_COLOR_TO, wxT("Select fancy tab style 'to' color"), wxT("Select fancy tab style 'to' color"), wxITEM_NORMAL);
	m_editMenu->Append(item);

	item = new wxMenuItem(styleMenu, MENU_SELECT_GRADIENT_COLOR_BORDER, wxT("Select fancy tab style 'border' color"), wxT("Select fancy tab style 'border' color"), wxITEM_NORMAL);
	m_editMenu->Append(item);
	...
	item = new wxMenuItem(m_editMenu, MENU_DRAW_BORDER, wxT("Draw Border around tab area"), wxT("Draw Border around tab area"), wxITEM_CHECK);
	m_editMenu->Append(item);
	item->Check(true);

	item = new wxMenuItem(m_editMenu, MENU_DRAW_TAB_X, wxT("Draw X button On Active Tab"), wxT("Draw X button On Active Tab"), wxITEM_CHECK);
	m_editMenu->Append(item);
	
	item = new wxMenuItem(styleMenu, MENU_SET_ACTIVE_TAB_COLOR, wxT("Select Active Tab Color"), wxT("Select Active Tab Color"), wxITEM_NORMAL);
	m_editMenu->Append(item);

	item = new wxMenuItem(styleMenu, MENU_SET_TAB_AREA_COLOR, wxT("Select Tab Area Color"), wxT("Select Tab Area Color"), wxITEM_NORMAL);
	m_editMenu->Append(item);


	item = new wxMenuItem(styleMenu, MENU_SET_ACTIVE_TEXT_COLOR, wxT("Select active tab text color"), wxT("Select active tab text color"), wxITEM_NORMAL);
	m_editMenu->Append(item);

	item = new wxMenuItem(styleMenu, MENU_SELECT_NONACTIVE_TEXT_COLOR, wxT("Select NON-active tab text color"), wxT("Select NON-active tab text color"), wxITEM_NORMAL);
	m_editMenu->Append(item);
	...
	item = new wxMenuItem(m_editMenu, MENU_GRADIENT_BACKGROUND, wxT("Use Gradient Coloring for tab area"), wxT("Use Gradient Coloring for tab area"), wxITEM_CHECK);
	m_editMenu->Append(item);
	item->Check( false );

	item = new wxMenuItem(m_editMenu, MENU_COLORFULL_TABS, wxT("Colorful tabs"), wxT("Colorful tabs"), wxITEM_CHECK);
	m_editMenu->Append(item);
	item->Check( false );
	...
	book = new wxStyledNotebook(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, bookStyle);
	secondBook = new wxStyledNotebook(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, bookStyle);
...
void Frame::OnAddPage(wxCommandEvent& WXUNUSED(event))
{
	...
	caption.Printf(wxT("New Page Added #%d"), newPageCounter++);
	Freeze();
...
{
  wxTextEntryDialog dlg(this, wxT("Enter Tab Number to select:"), wxT("Set Selection"));
    if(dlg.ShowModal() == wxID_OK)
      {
...
{
  wxTextEntryDialog dlg(this, wxT("Enter Tab Number to enable:"), wxT("Enable Tab"));
	if(dlg.ShowModal() == wxID_OK)
	{
		wxString val = dlg.GetValue();
		long tabIdx;
...
{
  wxTextEntryDialog dlg(this, wxT("Enter Tab Number to disable:"), wxT("Disable Tab"));
	if(dlg.ShowModal() == wxID_OK)
	{
		wxString val = dlg.GetValue();
		long tabIdx;
...
void Frame::OnSetPageImageIndex(wxCommandEvent& WXUNUSED(event))
{
	...
	wxTextEntryDialog dlg(this, 
		wxString::Format(wxT("Enter an image index (0-%i):"), (int)book->GetImageList()->size()-1), 
		wxT("Set Angle"));
	if(dlg.ShowModal() == wxID_OK)
	{
		wxString val = dlg.GetValue();
...
#ifdef DEVELOPMENT
	wxString msg;
	msg.Printf(wxT("Page is changing: old=%ld new=%ld"), event.GetOldSelection(), event.GetSelection());
	wxLogMessage(msg);
#endif
}

void Frame::OnPageChanged(wxFlatNotebookEvent& WXUNUSED(event))
{
#ifdef DEVELOPMENT
	wxString msg;
...
void Frame::OnPageClosing(wxFlatNotebookEvent& WXUNUSED(event))
{
#ifdef DEVELOPMENT
	wxString msg;

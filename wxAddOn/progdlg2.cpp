...
// Author:      Karsten Ballüder - modif by Pierre Fillard
// Modified by:
// Created:     09.05.1999
// RCS-ID:      $Id: progdlgg.cpp,v 1.98.4.1 2007/03/06 10:06:44 JS Exp $
...
BEGIN_EVENT_TABLE(wxProgressDialog2, wxDialog)
    EVT_BUTTON(wxID_CANCEL, wxProgressDialog2::OnCancel)
    EVT_BUTTON(wxID_SKIP, wxProgressDialog2::OnSkip)

    EVT_CLOSE(wxProgressDialog2::OnClose)
END_EVENT_TABLE()

IMPLEMENT_CLASS(wxProgressDialog2, wxDialog)

// ============================================================================
// wxProgressDialog2 implementation
// ============================================================================

// ----------------------------------------------------------------------------
// wxProgressDialog2 creation
// ----------------------------------------------------------------------------

wxProgressDialog2::wxProgressDialog2(wxString const &title,
...
wxStaticText *wxProgressDialog2::CreateLabel(const wxString& text,
                                            wxSizer *sizer)
{
...
// ----------------------------------------------------------------------------
// wxProgressDialog2 operations
// ----------------------------------------------------------------------------

bool
wxProgressDialog2::Update(int value, const wxString& newmsg, bool *skip)
{
...
bool wxProgressDialog2::Pulse(const wxString& newmsg, bool *skip)
{
...
bool wxProgressDialog2::DoAfterUpdate(bool *skip)
{
...
bool wxProgressDialog2::Show( bool show )
{
  if ( show && ( m_elapsed || m_estimated || m_remaining ) )
  {
    // set it to the current time
    m_timeStart = wxGetCurrentTime();
  }
...
void wxProgressDialog2::OnCancel(wxCommandEvent& event)
{
    if ( m_state == Finished )
    {
...
void wxProgressDialog2::OnSkip(wxCommandEvent& WXUNUSED(event))
{
    DisableSkip();
    m_skip = true;
}

void wxProgressDialog2::OnClose(wxCloseEvent& event)
{
...
wxProgressDialog2::~wxProgressDialog2()
{
    // normally this should have been already done, but just in case
    ReenableOtherWindows();
}

void wxProgressDialog2::ReenableOtherWindows()
{
...
void wxProgressDialog2::EnableSkip(bool enable)
{
    if(m_hasSkipButton)
    {
...
void wxProgressDialog2::EnableAbort(bool enable)
{
    if(m_hasAbortButton)
    {
#ifdef __SMARTPHONE__
...
void wxProgressDialog2::EnableClose()
{
    if(m_hasAbortButton)
    {
#ifdef __SMARTPHONE__
...
void wxProgressDialog2::UpdateMessage(const wxString &newmsg)
{
